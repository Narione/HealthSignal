...
const QnaView = () => {
  ...
  const deleteQuestion = async () => {
    await axios
      .get(`/api/question/delete?queNo=${queNo}`)
      .then((res) => {
        if (res.data === 1) {
          navigate("/qna/list");
        }
  ...
  //삭제버튼 생성하기
  useEffect(() => {
    if(privateInfo && userInfo){
      if(privateInfo.userNo == userInfo.userNo){
        setIsLoginUser(true);
      }
    }
  }, [question, userInfo]);
  ...
  useEffect(() => {
    if (privateInfo != null) {
      if (
        privateInfo.quePublic === "Y" ||
        privateInfo.userNo == userInfo.userNo ||
                    ...
                    <thead className="table-light">
                      ...
                      <tr>
                        ...
                        <th
                          ...
                          style={{ width: "35%", fontWeight: "normal" }}
                        >
                          {moment(question.queCreDate).format(
                            "YYYY-MM-DD HH:mm",
                          )}
                        </th>
                      </tr>
                      ...
                      <tr>
                        <td
                          style={{
                            ...
                            transition:
                              "max-height 0.3s ease" /* 높이 변경 시 애니메이션 추가 */,
                          }}
                          colSpan="4"
                        >
                  ...
                  <table className="mb-0 table">
                    <thead className="table-light">
                      <tr>
                        ...
                        >
                          {moment(answer.ansCreDate).format("YYYY-MM-DD HH:mm")}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
